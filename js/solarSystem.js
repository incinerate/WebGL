	...
	var getHTML = function (sun, planet) {
		...
		if (planet.satellites && planet.satellites.length > 0) {
			...
			html += Object.keys(planet.satellites).map((key) => {
				return '<li><a href="#' + getPath(sun, planet.satellites[key]) + '">' + planet.satellites[key].name + 
				'</a><input type="checkbox" id= "'+planet.satellites[key].name+'" checked = '+planet.satellites[key].checkStatus+' /></li>'
			}).join("");
			html += "</ol></dd>";
		}
...
//		console.log(html);
	...
	var main = function (shaderSources, textures) { // main()
		...
		window.onhashchange = function () {
			...
			selected = findPlanet(sun, hash) || findPlanet(sun, 'Sun');
		...
		var draw = function (prev, now, fps) {
			...
			var drawPlanet = function (planet) {
				...
				if (planet.satellites)
					planet.satellites.forEach(function (satellite) {
						if(null == document.getElementById(satellite.name) || document.getElementById(satellite.name).checked)
	...
	loadFiles({
			...
			loadImages({
				...
				phobos: "texture/mars-phobos.jpg",
